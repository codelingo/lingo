...
func (*commonSuite) TestWriteAndReadTenetCfg(c *gc.C) {
	...
	ctx := testhelper.MockContext(c, common.GlobalOptions, common.TenetCfgFlg.LongArg(), fName)
	c.Assert(common.WriteConfigFile(ctx, &common.Config{}), jc.ErrorIsNil)

	obtained, err := common.ReadConfigFile(fName)
	c.Assert(err, jc.ErrorIsNil)

	c.Assert(obtained, gc.DeepEquals, &common.Config{})
}

func (*commonSuite) TestDesiredTenetCfgPath(c *gc.C) {
	ctx := testhelper.MockContext(c, common.GlobalOptions, common.TenetCfgFlg.LongArg(), "custom/cfg/path")
...
func (*commonSuite) TestTenetCfgPath(c *gc.C) {
	...
	for cfgPath, expected := range map[string]string{
		...
		ctx := testhelper.MockContext(c, common.GlobalOptions, common.TenetCfgFlg.LongArg(), cfgPath)
...
func (*commonSuite) TestMockContext(c *gc.C) {
	ctx := testhelper.MockContext(c, common.GlobalOptions, common.TenetCfgFlg.LongArg(), "custom/cfg/path", "add")
...
func complexConfig() *common.Config {
	return &common.Config{
		Cascade:  true,
		Include:  "*",
		Template: "template.md",
		TenetGroups: []common.TenetGroup{
			{
				Name: "default",
				Tenets: []common.TenetConfig{
					{
...
func (*commonSuite) TestAddTenet(c *gc.C) {
	...
	err := testConfig1.AddTenet(common.TenetConfig{
		Name:    "lingoreviews/new_in_group1",
		Options: map[string]interface{}{"a": true, "b": false},
	}, "Group1")
	...
	err = testConfig2.AddTenet(common.TenetConfig{
		Name:    "lingoreviews/new_in_newgroup",
		Options: nil,
	}, "NewGroup")
	...
	err = testConfig3.AddTenet(common.TenetConfig{
		Name:    "lingoreviews/space_after_forward_slash",
		Options: map[string]interface{}{},
	}, "Group2")
