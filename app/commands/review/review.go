...
func Review(opts Options) ([]*Issue, error) {
	return serviceReview(opts)
	// return mockReview(opts)
}

func serviceReview(opts Options) ([]*Issue, error) {
	svc, err := service.New()
	if err != nil {
		// TODO(waigani) use proper logger for this
		fmt.Fprint(os.Stderr, "fatal", "specify a single -appdash.addr, -lightstep.access.token or -zipkin.kafka.addr")
		os.Exit(1)
	}
...
func NewRange(filename string, startLine, endLine int) *IssueRange {
	start := &Position{
		Filename: filename,
